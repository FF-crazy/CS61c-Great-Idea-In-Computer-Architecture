...
#pragma omp parallel
  {
    ...
    for (int i = thread_id; i < ARRAY_SIZE; i += thread_id) {
      z[i] = x[i] + y[i];
    }
  }
}
...
#pragma omp parallel
  {
    int num_thread = omp_get_num_threads();
    int thread_id = omp_get_thread_num();
    int time = ARRAY_SIZE / num_thread;
    flag_a = time;
    flag_b = num_thread;
    for (int i = thread_id * time; i < (thread_id + 1) * time; i++) {
      z[i] = x[i] + y[i];
    }
  }
  if (flag_a * flag_b != ARRAY_SIZE) {
    for (int i = flag_a * flag_b; i < ARRAY_SIZE; i++) {
      z[i] = x[i] + y[i];
    }
  }
}
